...
impl K8sClient {
    ...
    ///
    /// Whether the label shared with every other install may be taken away is read
    /// from the marks those installs write concurrently, so an unconditional write
    /// acts on a view of the node that has already moved on: two uninstalls could
    /// each see the other's mark, each remove only its own, and leave a node
    /// advertising Kata with nothing installed.
    ///
    ...
    pub async fn own_daemonset_exists(&self, daemonset_name: &str) -> Result<bool> {
        ...
        match ds_api.get_opt(daemonset_name).await? {
            Some(ds) if ds.metadata.deletion_timestamp.is_none() => Ok(true),
            None => Ok(false),
            Some(_) => Ok(false),
        }
    }
...
fn daemonset_selects_node(daemonset: &k8s_openapi::api::apps::v1::DaemonSet, node: &Node) -> bool {
    ...
    if !pod_spec.node_selector.as_ref().is_none_or(|selector| {
        selector
            .iter()
