...
async fn node_still_selected(client: &Client, args: &Args, node: &str) -> Result<bool> {
    ...
    for selector in selector_passes(&args.node_selector) {
        let params = ListParams::default()
            ...
            .labels(selector.unwrap_or_default())
            .limit(1);
        if !api
            .list(&params)
            .await
...
async fn run_fanout(
    ...
    loop {
        while in_flight.len() + post_work.len() < parallelism {
            ...
            if !args.removed_node_cleanup && !still_selected {
                error!(
                    "node {node}: no longer matches the node selection immediately before \
                     dispatch; refusing to mutate stale selection"
                );
                failed.push(node.to_string());
                continue;
            }
            ...
            if admitted.is_empty() {
                error!(
                    "node {node}: acquired untolerated taint {} before dispatch; refusing to start \
                     a Job that the taint manager would evict",
                    skipped
                        .first()
        ...
        // Poll each in-flight Job concurrently via GET so one slow apiserver
        // request cannot hold every other node behind it. Each request has its
        // own short timeout; the larger budget below permits transient failures
        // without allowing one unreadable Job to keep the rollout alive forever.
