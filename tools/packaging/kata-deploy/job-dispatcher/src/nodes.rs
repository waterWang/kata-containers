...
/// Every install also marks the nodes it installed on with a label of its own,
/// named after its multiInstallSuffix. [`KATA_RUNTIME_LABEL`] is shared by all of
...
enum SharedLabel {
    ...
    Keep,
    /// Another install is mid-flight or failed here. Nothing may be scheduled on
    /// the strength of it, but the key has to stay so that install's own uninstall
...
impl NodeOps {
    ...
    async fn demote(&self, node: &str) -> Result<()> {
        self.rewrite_labels(node, |labels, ours| {
            ...
            // Our own RuntimeClasses select our mark, so demoting it is what stops
            // this install's workloads reaching the node. The shared label is a
            // different question: another install serving Kata here still needs it.
    ...
    async fn release(&self, node: &str) -> Result<()> {
        self.rewrite_labels(node, |labels, ours| {
            ...
            match shared_label_after(labels, ours) {
                ...
                // Someone else's unfinished install: it needs the key to find this
                // node again, but nothing may be scheduled here meanwhile.
                SharedLabel::Demote => {
                        ...
                        != Some(KATA_RUNTIME_PENDING)
                    {
                        info!(
                            "node {node}: leaving {KATA_RUNTIME_LABEL}={KATA_RUNTIME_PENDING}, \
                             another kata-deploy install has claimed this node but has not \
                             finished installing on it"
                        );
                        updates.push((
                            KATA_RUNTIME_LABEL.to_string(),
                            Some(KATA_RUNTIME_PENDING.to_string()),
    ...
    ///
    /// Both decisions this drives - whether the shared label may go, and whether it
    /// may say `true` - are read from labels other installs write concurrently. An
    /// unconditional write would act on a view of the node that has already moved
    /// on: two uninstalls could each see the other's mark and each remove their own,
    /// leaving a node advertising Kata with nothing installed.
    ...
    async fn label_until_stable(&self, node: &str, value: &str) -> Result<()> {
        // Both labels, because both gate scheduling: the RuntimeClasses of this
        // install select our marker as well as the shared label, so a marker the
        // kubelet clobbered leaves the node advertised and unusable by us.
