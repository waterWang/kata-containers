...
@test "Helm template: the default install's RuntimeClasses select its mark too" {
	# "Default" is still one installation when another suffixed release shares the
	# node. Without its mark in the selector, removing default while the other
	# release preserves the shared label keeps default workloads schedulable onto a
	# node whose default runtime is being removed.
	local rendered
	rendered=$(helm template kata-deploy "${CHART_PATH}" \
		--set deploymentMode=job \
		--show-only templates/runtimeclasses.yaml)
