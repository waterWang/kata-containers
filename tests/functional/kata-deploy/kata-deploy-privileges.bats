...
# job-templates ConfigMap: from the pod template's `spec:` up to the next key at
# the same depth.
#
# Grepping the whole ConfigMap cannot tell a pod-level key from a container-level
# one, which is exactly the difference between a pod that has no token and a pod
# that has one.
per_node_pod_spec() {
	local stage="${1}"
	shift
...
# One YAML document out of a rendered multi-document template, picked by the
# metadata name it declares. Lets a test assert what a single ClusterRole grants
# instead of counting substrings across every document at once.
rbac_doc() {
...
@test "Helm template (job mode): the pods that run on every node carry no token" {
	...
	# so omitting serviceAccountName is not by itself enough. The key only counts
	# where kubelet reads it: in the pod spec of each stage's Job.
...
@test "Helm template (job mode): nothing can keep a cleanup pod off a node" {
	# Uninstall has to reach every node the install touched. The dispatcher pins
	# these pods by name, which gets them past the scheduler - but a NoExecute
	# taint added since the install would evict a bound pod all the same, leaving
	# the node modified and out of reach for good.
	local cleanup_spec install_spec
	cleanup_spec=$(per_node_pod_spec cleanup)
	echo "${cleanup_spec}" | grep -qE '^          tolerations:$'
	echo "${cleanup_spec}" | grep -qE '^            - operator: Exists$'

	# The install is the opposite case: where it may land is the operator's call,
	# so it carries their tolerations and nothing else.
	install_spec=$(per_node_pod_spec install --set 'tolerations[0].key=kata' \
		--set 'tolerations[0].operator=Exists')
	echo "${install_spec}" | grep -qE '^            - key: kata$'
	refute_match "${install_spec}" '- operator: Exists'
}
...
@test "Helm template (job mode): the dispatcher requires the handlers this release installs" {
	# What a runtime *loaded* only the node can answer, and answering needs the
	# apiserver - so the dispatcher asks before it labels the node. Asking for a
	# handler name nothing ever serves would fail every node, so the names have to
	# be the ones the chart itself registers RuntimeClasses for.
	...
	# A release whose only runtimes are custom ones still gets checked: the names
	# come from the RuntimeClasses the user supplied. Skipping the check here would
	# label these nodes on nothing but a stage's exit code.
...
@test "Helm template (job mode): the dispatcher may patch nodes, and nothing else may" {
	...
	# The rights over nodes belong to the dispatcher's ClusterRole and to no other
	# document, so read that one document rather than the whole file: a rule that
	# drifted into another role would otherwise still match.
	...
	# Per-node Jobs, on the other hand, are created and deleted - a Job left by an
	# earlier run has to be replaced, not adopted - and only in one namespace.
	local role
